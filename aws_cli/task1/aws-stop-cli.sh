...
web1=$(aws ec2 describe-instances --filters "Name=tag:Name,Values=web-1" "Name=instance-state-name,Values=running" --query 'Reservations[0].Instances[0].InstanceId' --output text)

web2=$(aws ec2 describe-instances --filters "Name=tag:Name,Values=web-2" "Name=instance-state-name,Values=running" --query 'Reservations[0].Instances[0].InstanceId' --output text)

aws ec2 terminate-instances --instance-ids $web1
aws ec2 terminate-instances --instance-ids $web2
...
aws ec2 wait instance-terminated --instance-ids $web1
aws ec2 wait instance-terminated --instance-ids $web2
...
for i in $db_sg $web_sg ; do aws ec2 delete-security-group --group-id $i; done
